...
const STEPS = [
  ...
  { number: 3, label: 'Company Match' },
  { number: 4, label: 'Interview Prep' },
  { number: 5, label: 'Mock Interview' },
]
...
export default function Stepper({ currentStep }) {
  ...
  return (
    <div className="mb-8 flex flex-col items-center gap-3">
          ...
          return (
            <div key={step.number} className="flex items-center">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span
                  className={`hidden text-[11px] font-semibold whitespace-nowrap sm:block ${
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`mx-2 mb-5 h-0.5 w-6 rounded-full sm:mx-3 sm:w-12 md:w-16 ${
      ...
      {activeStep && (
        <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 sm:hidden">
          Step {activeStep.number} of {STEPS.length}: {activeStep.label}
        </span>
      )}
    </div>
  )
